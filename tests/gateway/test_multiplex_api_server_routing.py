...
class TestApiServerScopeProfile:
    """Under multiplex, EVERY request must be scoped — never a no-op —
    or the fail-closed get_secret() guard raises UnscopedSecretError
    (the crash Fadeway hit on /v1/chat/completions).
    """

    def test_no_prefix_scopes_to_default_when_multiplex_on(self, monkeypatch):
        adapter = _make_adapter(multiplex=True)
        monkeypatch.setattr("agent.secret_scope.is_multiplex_active", lambda: True)
        monkeypatch.setattr(
            "hermes_cli.profiles.get_active_profile_name", lambda: "default"
        )
        # No /p/ prefix, but multiplex is armed → scope to default, not None.
        assert adapter._resolve_scope_profile(_FakeReq(None)) == "default"

    def test_prefix_scopes_to_named_profile(self, monkeypatch):
        adapter = _make_adapter(multiplex=True)
        monkeypatch.setattr("agent.secret_scope.is_multiplex_active", lambda: True)
        monkeypatch.setattr(
            "hermes_cli.profiles.profiles_to_serve",
        ...
        assert adapter._resolve_scope_profile(_FakeReq("coder")) == "coder"

    def test_unknown_prefix_rejected(self, monkeypatch):
        adapter = _make_adapter(multiplex=True)
        monkeypatch.setattr("agent.secret_scope.is_multiplex_active", lambda: True)
        monkeypatch.setattr(
            "hermes_cli.profiles.profiles_to_serve",
        ...
        assert adapter._resolve_scope_profile(_FakeReq("ghost")) is _PROFILE_REJECTED

    def test_multiplex_off_is_noop_scope(self, monkeypatch):
        adapter = _make_adapter(multiplex=False)
        monkeypatch.setattr("agent.secret_scope.is_multiplex_active", lambda: False)
        # Single-profile: no scoping, unchanged behaviour.
        assert adapter._resolve_scope_profile(_FakeReq(None)) is None
        assert adapter._resolve_scope_profile(_FakeReq("coder")) is None

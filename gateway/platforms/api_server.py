...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _multiplex_active(self) -> bool:
        """Whether this process runs as a profile multiplexer.

        ``is_multiplex_active()`` is the authoritative signal — it is the SAME
        process-global the fail-closed ``get_secret`` guard consults, set once
        at gateway startup. Using it here keeps our scoping and that guard in
        lockstep: if the guard is armed, every request path is scoped too. The
        gateway_runner config is only a fallback for unit tests that construct
        an adapter without arming the process flag.
        """
        try:
    ...
    def _resolve_scope_profile(self, request: "web.Request"):
        """Profile to scope the whole request to, or ``_PROFILE_REJECTED``.

        With multiplexing OFF this returns ``None`` (a no-op scope — single
        profile behaviour is unchanged). With multiplexing ON *every* request
        must run inside a secret scope, or the fail-closed ``get_secret`` guard
        raises ``UnscopedSecretError``. So we resolve to the prefixed profile
        when present, otherwise to the multiplexer's own (default/active)
        profile — never a no-op scope.
        """
        ...
        if resolved:
            return resolved
        # No prefix under multiplex → serve as the default/active profile.
        try:
            from hermes_cli.profiles import get_active_profile_name
    ...
    def _profile_scope(profile: Optional[str]):
        """Enter the profile runtime scope, or a no-op when unset.

        A no-op is correct ONLY when multiplexing is off; callers pass ``None``
        in that case. When multiplexing is on they pass a concrete profile name
        (the prefixed one, or the default), so a real secret scope is always
        installed and ``get_secret`` never fails closed.
        """
        if not profile:
            return nullcontext()
    ...
    def _make_profile_prefix_middleware(self):
        ...
        async def profile_prefix_middleware(request: "web.Request", handler):
            scope_profile = self._resolve_scope_profile(request)
            if scope_profile is _PROFILE_REJECTED:
                return web.json_response(
                    {"error": "Unknown or unconfigured profile"},
                    status=404,
                )
            token = _api_request_profile.set(scope_profile)
            try:
                with self._profile_scope(scope_profile):
                    return await handler(request)
            finally:
